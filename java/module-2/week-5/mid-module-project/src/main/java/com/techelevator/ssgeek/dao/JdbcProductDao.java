...
public class JdbcProductDao implements ProductDao {
    ...
    public List<Product> getProductsWithNoSales() {
        ...
        String sql = "SELECT product.product_id, name, description, price, image_name " +
                "FROM product " +
                "LEFT JOIN line_item ON line_item.product_id = product.product_id " +
                "WHERE sale_id IS null " +
                "ORDER BY product.product_id";
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql);
        while (results.next()) {
            Product product = mapRowToProduct(results);
            productsWithoutSales.add(product);
        }
    ...
    public void updateProduct(Product updatedProduct) {
        String sql = "UPDATE product " +
                ...
                "WHERE product_id = ?";
        jdbcTemplate.update(sql, updatedProduct.getName(), updatedProduct.getDescription(), updatedProduct.getPrice(), updatedProduct.getImageName(),
